...
st.caption("게임 화면을 '클릭'하여 마우스를 고정한 후 플레이하세요!")

st.markdown("""
| 동작 | 키보드 / 마우스 조작 |
| :--- | :--- |
| **시점 및 고정** | 게임 화면 클릭 (해제: `ESC`) |
| **이동** | `W` (앞), `S` (뒤), `A` (왼쪽), `D` (오른쪽) |
| **점프** | `Space` 키 |
| **슛 던지기** | **마우스 왼쪽 버튼 (누르고 있으면 파워 장전, 떼면 슛!)** |
""")
